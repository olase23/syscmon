 ...
 */
int scm_table_change_add(struct module	*mod, unsigned long origin, unsigned long new_ptr, u16 syscall_nr) {
...
static inline void scm_read_idt_entry(gate_desc *gate, unsigned int vector, unsigned long *idt, unsigned short size)
{
    if ((size) && (vector < size))
...
//	int										_exit		= 0;
  ...
  syscall_func = native_read_msr(MSR_LSTAR);
	...
	for(;;) {
    	...
    	for( i=0; i < NR_syscalls; i++ ) {
				...
				if((!scm_find_list_item((unsigned long) sys_call_table[i], i)) &&
						((unsigned long *) sys_call_table[i] == (unsigned long *) shadow_sys_call_tbl[i]))
							continue;
				...
				scm_table_change_add(kmodule, (unsigned long ) shadow_sys_call_tbl[i],(unsigned long) sys_call_table[i], i);

				if (!kmodule) {

	    		printk(KERN_WARNING "sysc_mon: Attantion entry in the sys call table was changed: \n"
      ...
      if(ia32_addr != ia32_sysenter_func){
        ...
        if(ia32_addr) {
          ...
          if (!kmodule) {
            ...
            printk(KERN_WARNING "sysc_mon: Attantion MSR_IA32_SYSENTER_EIP was changed: \n"
    ...
    /*
		 * check consistency of the SYSCALL instruction setup in 64-bit flat code segment mode
		 */
...
static int __init _init_sysc_mon(void) {
    ...
    if (IS_ERR(scm_task)) {
      ...
      printk(KERN_ERR "scm_: disabled - Unable to start kernel thread. error=%u\n", error);
      return 0;
    }
